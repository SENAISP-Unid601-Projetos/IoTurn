...
const createMachineBodySchema = z.object({
  ...
  responsibleUserId: z
    .number()
    .positive('O ID do responsavel deve ser um número positivo.')
    .optional(),
...
const clientAndMachineParamsSchema = z.object({
  clientId: z.coerce.number().int().positive('O ID do cliente deve ser um número positivo.'),
  machineId: z.coerce.number().int().positive('O ID da máquina deve ser um número positivo.'),
})
...
export const machineController = {
  createMachineController: async (request: FastifyRequest, reply: FastifyReply): Promise<void> => {
    try {
      ...
      if (!machineData.success) {
        reply
          .status(400)
          .send({ message: 'Invalid request body', errors: machineData.error.message })
        return
      }
  ...
  getAllUsersMachineController: async (
    ...
    try {
      ...
      if (!paramsValidation.success) {
        reply
          .status(400)
          .send({ message: 'Invalid request parameters', errors: paramsValidation.error.message })
        return
      }
      ...
      return reply
        .status(500)
        .send({ error: 'Erro interno do servidor ao buscar máquinas do usuário.' })
    }
  },
  updateMachine: async (request: FastifyRequest, reply: FastifyReply): Promise<void> => {
    try {
      ...
      if (!paramsValidation.success) {
        reply
          .status(400)
          .send({ message: 'Parâmetro [Id] inválido', errors: paramsValidation.error.message })
        return
      }
      ...
      if (!packageData.success) {
        reply
          .status(400)
          .send({ message: 'Invalid request body', errors: packageData.error.message })
        return
      }
  ...
  getMachineByClientAndIdController: async (
    request: FastifyRequest,
    reply: FastifyReply
  ): Promise<void> => {
    try {
      const paramsValidation = clientAndMachineParamsSchema.safeParse(request.params)

      if (!paramsValidation.success) {
        reply.status(400).send({
          message: 'Parâmetros inválidos',
          errors: paramsValidation.error.message,
        })
        return
      }

      const { clientId, machineId } = paramsValidation.data
      const machine = await machineService.getMachineByClientAndId(clientId, machineId)

      if (!machine) {
        reply.status(404).send({ message: 'Máquina não encontrada para este cliente.' })
        return
      }

      return reply.status(200).send(machine)
    } catch (error) {
      console.error('Erro ao buscar máquina por cliente e id:', error)
      return reply.status(500).send({ error: 'Erro interno do servidor ao buscar máquina.' })
    }
  },
}
