...
export const machineRepository = {
  ...
  findMachineByClientAndId: async (clientId: number, machineId: number) => {
    try {
      ...
      if (!machine) throw new Error('Máquina não encontrada para o cliente informado.')

      return machine
    } catch (err) {
      console.error('Erro ao buscar máquina por cliente e id:', err)
      throw new Error('Falha ao acessar o banco de dados para buscar a máquina.')
    }
  },
}
